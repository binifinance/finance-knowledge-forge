...
const Index = () => {
  ...
  const filteredArticles = sampleArticles.filter(article => {
    const matchesSearch = searchTerm === '' || article.title.toLowerCase().includes(searchTerm.toLowerCase()) || article.body.toLowerCase().includes(searchTerm.toLowerCase()) || article.tags?.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="ml-0 lg:ml-[260px] min-h-screen">
          ...
          <div className="max-w-6xl mx-auto px-6 py-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search articles, topics, or tags..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-input border border-border rounded-md text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent" />
              </div>
              
              <div className="relative">
                <Filter size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" />
                <select value={filterSection} onChange={e => setFilterSection(e.target.value as 'All' | 'Accounting' | 'Finance')} className="pl-10 pr-8 py-2.5 bg-input border border-border rounded-md text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent appearance-none cursor-pointer">
        ...
        {/* Articles Content */}
        <div className="max-w-6xl mx-auto p-6 text-gray-900" style={{
        background: 'radial-gradient(ellipse at center, white 40%, #0B2545 100%)'
          ...
          {filteredArticles.length === 0 ? <div className="text-center py-12">
              ...
              <div className="grid gap-6">
                {filteredArticles.map(article => <ArticleBlock key={article.id} article={article} onReadMore={handleReadMore} />)}
              </div>
            </>}
        </div>
      ...
      {/* Article Modal */}
      <ArticleModal article={selectedArticle} onClose={handleCloseModal} relatedArticles={selectedArticle ? getRelatedArticles(selectedArticle) : []} />
    </div>;
};
